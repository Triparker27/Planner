/* planner.js
   Adds Tests & Quizzes tab and renders quiz events on calendar.
   Classes: calendar shows only icons; clicking a day opens modal listing classes for that weekday.
*/
...
function updateColorPreview(){
  ...
  if(sel && $('colorPreview')) $('colorPreview').style.background = sel.value;
}
...
function createClassEntry(name, color, days, start, end){
  ...
  div.innerHTML = `
    ...
    <div>
      <button class="primary" onclick="deleteClass('${escapeJs(finalName)}')">Delete</button>
    </div>
  `;
...
function deleteClass(name){
  document.querySelectorAll('#classList .class-preview').forEach(n=>{
    if(n.getAttribute('data-class') === name) n.remove();
  });
  ['assignmentClass','quizClass'].forEach(id=>{
    const sel = $(id);
    if(!sel) return;
...
/* Calendar rendering: classes show icons; quizzes and assignments show on dates */
function renderCalendar(){
  ...
  for(let day=1; day<=daysInMonth; day++){
    ...
    // Classes for this weekday (exclude 'Online' from calendar)
    ...
    if(classes.length > 0){
      classes.slice(0,2).forEach(c => {
        ...
        icon.title = `${c.name} • ${c.start} - ${c.end}`;
    ...
    Array.from(document.querySelectorAll('#quizList .quiz-preview')).forEach(q => {
      ...
      if(qDate === isoDate){
        ...
        icon.title = `${q.getAttribute('data-quiz')} • ${q.getAttribute('data-time') || ''}`;
        icon.textContent = '⚠️';
        cell.appendChild(icon);
      }
    });
    ...
    Array.from(document.querySelectorAll('#assignmentList .assignment-preview')).forEach(a => {
      const aDate = a.querySelector('.class-meta') ? extractDateFromMeta(a) : '';
      if(aDate === isoDate){
        const ev = document.createElement('div');
        ev.className = 'quiz-icon';
    ...
    // click handler opens class modal for that weekday
...
/* Modal: show classes for clicked day (only classes, with times) */
...
/* Extract date from assignment meta (best-effort) */
function extractDateFromMeta(node){
  // assignment meta format: "Class • YYYY-MM-DD • HH:MM" or similar
  const meta = node.querySelector('.class-meta');
  if(!meta) return '';
